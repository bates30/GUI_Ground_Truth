...
def get_bipartitions_from_graph(G, root, all_tips):
    ...
    # For each internal node (junction or cluster), find all leaves in its subtree
    for node in G.nodes():
        if G.nodes[node]['kind'] in ['junction', 'cluster']:
            # Get all descendants
...
def visualize_trees_with_partition_coloring(bible_tree, test_tree, bible_root, test_root,
                                            ...
                                            rf_distance, max_rf, output_path):
    ...
    # Add main title with metrics - Max RF
    fig.suptitle(f'RF Distance: {rf_distance} | Max RF: {max_rf}',
                 fontsize=32, fontweight='bold', y=0.98)  # Larger title

    fig.legend(handles=legend_items, loc='lower center', fontsize=16, ncol=3,
               bbox_to_anchor=(0.5, -0.01))  # Smaller legend

    plt.tight_layout()
    fig.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close(fig)

def save_metrics_json(rf_distance, max_rf, output_dir):

    # Calculate similarity and save/update Metrics.json in the output directory.
    # Similarity = 1 - (RF distance / RF max)

    # Calculate similarity
    if max_rf > 0:
        similarity = 1 - (rf_distance / max_rf)
    else:
        similarity = 1.0  # If max_rf is 0, trees are identical
    ...
    # Update with RF similarity
    metrics_data["RF_Similarity"] = similarity
...
def main(generate_image=True, GT_Tree = "Ground_Truth_CB1.json", Test_Tree = "CMST_CB1.json"):
    #Calculate RF distance similarity and optionally generate visualization.
    ...
    # Calculate max RF distance using: 2×common_splits + bible_only_splits + test_only_splits
    max_rf = 2 * len(metrics['common']) + len(metrics['bible_only']) + len(metrics['test_only'])
    ...
    if generate_image:
        ...
        visualize_trees_with_partition_coloring(
            ...
            common_leaves, bible_only_leaves, test_only_leaves,
            metrics['rf_distance'], max_rf, output_path
        )

    # Save similarity metric to Metrics.json
    save_metrics_json(metrics['rf_distance'], max_rf, output_dir)
